...
module "sql-server-02" {
  source              = "../modules/sql_server"
  sql_server_name     = "sql-server-0269"
  location            = local.location
  resource_group_name = azurerm_resource_group.rg.name
  tags                = local.tags
}

module "sql-database-02" {
  source              = "../modules/sql_database"
  sql_database_name   = "sql-database-0269"
  tags                = local.tags
  location            = local.location
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_mssql_virtual_network_rule" "sql_vnet_integration-02" {
  name      = "sql-vnet-rule"
  server_id = module.sql-server-02.id
